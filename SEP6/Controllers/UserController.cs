...
namespace SEP6.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public ObjectResult GetUser([FromHeader] string fromToken, string toUser)
        {
            ...
            {
                return Ok(_dbContext.Users.Include(a=>a.TopLists).ThenInclude(toplist=>toplist.Movies).Include(followers => followers.Followers).Include(follows => follows.Follows).AsParallel().FirstOrDefault(a => a.Username == fromUser.Username));
            }
            else
            {
                return Ok(_dbContext.Users.Include(a=>a.TopLists).ThenInclude(toplist=>toplist.Movies).Include(followers => followers.Followers).Include(follows => follows.Follows).AsParallel().FirstOrDefault(a => a.Username == toUser));
            }
        }
    }
}